...
risk_management_agent = Agent(
    ...
    backstory="This agent scrutinizes the potential risks of proposed trades and suggests safeguards to ensure trading activities align with the firm’s risk tolerance.",
    verbose=True,
    allow_delegation=True,
    tools=[scrape_tool, search_tool]
)
...
data_analysis_task = Task(
    description="Monitor and analyze market data for {stock_selection}. Use statistical modeling and machine learning to identify trends and predict market movements.",
    expected_output="Insights and alerts about significant market opportunities or threats for {stock_selection}.",
    agent=data_analyst_agent,
)

strategy_development_task = Task(
    description="Develop and refine trading strategies based on the insights from the Data Analyst and user-defined risk tolerance ({risk_tolerance}).",
...
stock_selection = st.text_input('Stock Selection', 'Apple')
initial_capital = st.text_input('Initial Capital', '100000')
risk_tolerance = st.selectbox('Risk Tolerance', ['Low', 'Medium', 'High'], index=1)
trading_strategy_preference = st.selectbox('Trading Strategy Preference', ['Day Trading', 'Swing Trading', 'Scalping'])
